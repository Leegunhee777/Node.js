//디비를 연결하여 app.js에 회원가입 기능을가지고 있는  예제임
//http://localhost:3000/public/adduser.html 접속하면 디비에 추가됨
...
function connectDB(){
    ...
    MongoClient.connect(databaseUrl,function(err,db){
        ...
        //database 변수에 할당
        database = db.db('local');//mongodb 3.0버전이상일때는 connection을 할때 database명을 명시해야한다.
    });
}
...
router.route('/process/login').post(function(req,res){
    ...
    if(database){
        ...
        res.write('<div><p>데이터베이스를 연결하지 못했습니다.</p></div>');
        res.end();
    }
});
...
router.route('/process/adduser').post(function(req,res){
    ...
    else { //데이터 베이스 객체가 초기화되지 않는 경우 실패 응답 전송
        res.writeHead('200',{'Content-Type':'text/html;charset=utf8'});
